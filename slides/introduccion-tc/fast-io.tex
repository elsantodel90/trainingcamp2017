...
\author[Agustín Gutiérrez y Juan Cruz Piñero] % (optional, use only with lots of authors)
...
\begin{frame}[fragile]{?`Cuál Python?}
  \textbf{Python2 $\neq$ Python3}\newline
  \begin{itemize}
  \item Diferencia 1: Compatibilidad Unicode \newline
  \item Diferencia 2: Instrucciones de Entrada \newline
  \item Diferencia 3: Instrucción de Salida \newline
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Compatibilidad Unicode}
  Python3 no diferencia entre string y unicode: \textbf{la representación interna de los strings es unicode.}
  Python2 en cambio, utiliza internamente codificación ascii al manipular strings, pero provee formas sencillas para codificar/descodificar cadenas unicode.
\end{frame}

\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Compatibilidad Unicode}
  \begin{itemize}
    \item Python2 permite obtener un string o un unicode a partir de un objeto (por ejemplo un número o una lista), obteniendo respectivamente una representación ascii o unicode.
  \end{itemize}
\begin{verbatim}
>>> a = "abcdefgñ....í....ó....uvwz"
>>> b = u"abcdefgñ....í....ó....uvwz"
>>> a
...
\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Compatibilidad Unicode}
  \begin{itemize}
    \item python 3 sólo nos permite obtener un string (que en verdad es un unicode)
  \end{itemize}
\begin{verbatim}
>>> a = "abcdefgñ....í....ó....uvwz"
>>> b = u"abcdefgñ....í....ó....uvwz"
    ...
    \item \textbf{Python2} $-$ input() vs raw\_input()
        ...
        \pause
            \invisible<1>{
                ...
                \item {\it input()} interpreta como una expresión python el string que se obtiene al leer la línea actual del stream de entrada. (Si no hay una expresión valida en python, da error)
            }
            \pause
            \invisible<1-2>{
            ...
            \pause
            \invisible<1-3>{
                \item ?`Qué es más eficiente y por qué?
            }
        \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Instrucciones de Entrada}
  \begin{itemize}
    \item \textbf{Python3} $-$ input() !`no hay más raw\_input()! \newline
        \pause
            \invisible<1>{
               \begin{figure}
               \includegraphics[scale=0.5]{estupido_el_unico_con_talento.jpg}
               \caption{¡Estúpido era la mejor forma de leer entrada en python!}
               \label{fig:simpsons1}
               \end{figure}
            }
  \end{itemize}
...
\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Instrucciones de Entrada}
  \begin{itemize}
    \item \textbf{Python3} $-$ input() !`no hay más raw\_input()! \newline
        ...
        Renombraron {\it raw\_input()} como {\it input()}, que es la intención más común, y el {\it input()} de python 2 puede ser sustituido con {\it eval(input())}. \newline
...
\begin{frame}[fragile]{Python2 $\neq$ Python3 $-$ Instrucción de Salida}
  \begin{itemize}
      \item \textbf{Python2} $-$ {\it print} como palabra reservada.
        ...
        \invisible<1>{
        \pause
        \item No es una llamada a función, es una palabra reservada que imprime la expresión a la derecha.
        }
        
        \invisible<1-2>{
            \pause
            \item \textbf{Python3} $-$ {\it print()} ¡Ahora en forma de función!.
            \newline !`Ahora ya no es una palabra reservada del lenguaje sino una funcióna la que hay que pasarle por parámetro lo que querramos imprimir!
        }
  \end{itemize}
\end{frame}
